...
with st.sidebar:
    ...
    if tipo == "Commessa":
        ...
        if st.button("Salva Ordine"):
            conn.table("ordini").insert({"commessa_id": nomi_c[scelta_c], "nome_ordine": nome_o}).execute()
...
dizionario_ordini = {o['nome_ordine']: o['id'] for o in tutti_ordini.data}
...
if st.button("Indicizza Documento"):
    if ordini_scelti and nome_doc and path_doc:
        ...
        res = conn.table("documenti").insert({"nome_documento": nome_doc, "percorso_locale": path_doc}).execute()
        doc_id = res.data[0]['id']
        
        # Inserisce i collegamenti nella tabella ponte
        for o_nome in ordini_scelti:
            conn.table("documento_ordini").insert({"documento_id": doc_id, "ordine_id": dizionario_ordini[o_nome]}).execute()
...
if ordine_filtro:
    ...
    docs = conn.table("documento_ordini").select("documenti(nome_documento, percorso_locale)").eq("ordine_id", o_id).execute()
    
    for item in docs.data:
        d = item['documenti']
        st.write(f"📄 **{d['nome_documento']}**")
        st.code(d['percorso_locale'])
